 ...
 */
type AllowedLanguageIds = 'asm-collection' | 'asm-list-file';
 ...
 */
function getGlobalIncludeForLanguageId(languageId: AllowedLanguageIds): string {
    if (languageId == 'asm-collection')
        return asmCollectionCache.getData();
    if (languageId == 'asm-list-file')
        return asmFileListCache.getData();
    // Should not reach here
    assert(false, 'languageId = ' + languageId + ' unknown.');
}
let asmCollectionCache = new FuncCache<string>(10000, () => {
    return _getGlobalIncludeForLanguageId('asm-collection');
});
let asmFileListCache = new FuncCache<string>(10000, () => {
    return _getGlobalIncludeForLanguageId('asm-list-file');
});
 ...
 * @returns  E.g. "** /*.{asm, inc, s}"
 */
function _getGlobalIncludeForLanguageId(languageId: AllowedLanguageIds): string {
    // Package json
...
export async function grep(regex: RegExp, rootFolder: string, languageId: string): Promise<GrepLocation[]> {
    ...
    try {
        const globInclude = getGlobalIncludeForLanguageId(languageId as AllowedLanguageIds);
