...
export function regexLabelColonNew(): RegExp {
    //    return /(^\s*@?)\b([a-z_][\w\.]*):/i;
 ...
 */
export function regexEveryLabelColonForWordForCompletion(fuzzySearchWord: string): RegExp {
 ...
 * @param fuzzySearchWord Is a fuzzy search word, e.g. "\\w*s\\w*n\\w*d" for snd.
 */
export function regexEveryLabelWithoutColonForWordForCompletion(fuzzySearchWord: string): RegExp {
 ...
 */
export function regexesEveryLabelForWordForCompletion(fuzzySearchWord: string, cfg: {labelsWithColons: boolean, labelsWithoutColons: boolean}): RegExp[] {
    const regexes: RegExp[] = [];
    // Find all "some.thing:" (labels) in the document
    if (cfg.labelsWithColons) {
        const searchRegex = regexEveryLabelColonForWordForCompletion(fuzzySearchWord);
        regexes.push(searchRegex);
    }
    // Find all sjasmplus labels without ":" in the document
    if (cfg.labelsWithoutColons) {
        const searchRegex2 = regexEveryLabelWithoutColonForWordForCompletion(fuzzySearchWord);
        regexes.push(searchRegex2);
    }
    return regexes;
}
